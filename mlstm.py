...
class mLSTM(nn.Module):
    ...
    def forward(self, input_seq, hidden_state=None):
        ...
        for t in range(seq_length):
            x = self.layer_norm(input_seq[:, t, :])
            queries = self.layer_norm(self.W_q(x))
            keys = self.layer_norm(self.W_k(x))
            values = self.layer_norm(self.W_v(x))
            ...
            for i, (lstm, dropout, i_gate, f_gate, o_gate) in enumerate(zip(self.lstms, self.dropout_layers, self.exp_input_gates, self.exp_forget_gates, self.output_gates)):
                ...
                C_t = f * C + torch.matmul(ii, torch.matmul(values.transpose(0, 1), keys))
                C_t = self.layer_norm(C_t)
                attn_output = queries * C_t
