...
class xLSTM(nn.Module):
    def __init__(self, vocab_size, embedding_size, hidden_size, num_layers, num_blocks, dropout=0.0, bidirectional=False, lstm_type="slstm"):
        ...
        self.blocks = nn.ModuleList([xLSTMBlock(hidden_size, hidden_size, num_layers, dropout, bidirectional, lstm_type) for i in range(num_blocks)])
    ...
    def forward(self, input_seq, hidden_states=None):
        ...
        output_seq = self.input_layer(input_seq)
        ...
        return output_seq[:, -1, :], hidden_states